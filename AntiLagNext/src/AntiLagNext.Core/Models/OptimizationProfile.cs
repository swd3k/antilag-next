...
public sealed class OptimizationProfile
{
    ...
    public static OptimizationProfile CreatePreset(ProfileKind kind) => kind switch
    {
        ProfileKind.Gaming => new OptimizationProfile
        {
            ...
            Description = "Max responsiveness: timer 0.5 ms, High Performance, all cores, Game Mode. HAGS and RAM-trim off (opt-in — they often add hitching).",
            EnableTimer = true,
            TimerTargetMs = 0.5,
        ...
        ProfileKind.MaxPerformance => new OptimizationProfile
        {
            ...
            Description = "Ultimate Performance when available, timer 0.5 ms, all cores, Game Mode/GPU LLM. HAGS and RAM-trim off by default.",
            EnableTimer = true,
            TimerTargetMs = 0.5,
            EnablePowerScheme = true,
