...
### Fixed
- **Tray Exit**: «Exit» from the tray menu now marshals `Close` onto the Photino UI thread (tray runs on WinForms STA) and hard-exits after 1.5s if close is swallowed — process no longer can stick with only a tray icon.
...
|---------|------------|------------|
| **1.4.1** | 2026-08-22 | SHA256 Setup verify, System32 tools, chip logo in app |
